   ...
   // String otpString;
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //  otpString = getIntent().getStringExtra(OTP);
        ...
        imeiString = prefs.getString(AppConstant.Preference.IMEI, "");
    ...
    private void otpRequest(final String userID, final String otp, final String physical_add, final ProgressDialog progressDialog) {
        StringRequest stringLoginRequest = new StringRequest(Request.Method.POST, AppConstant.API.OTP_VERIFY_API,
                            ...
                            if (status!=null && status.equalsIgnoreCase("1")) {
                                if (jsonObject.getString("Result").equalsIgnoreCase("success")) {
                                    ...
                                    if (jsonObject.has("message")) {
                                        ...
                                        if (failMessage.contains("already registered")) {
                                            insertAccountInfo(jsonObject);
                                            String accountName = jsonObject.has("user_name") ? jsonObject.getString("user_name") : "";
                                            onSignupSuccess("Contact No. already register with " + accountName + " name");
                                        } else {
                                            onSignupFailed(failMessage);
                                        }
                                    } else {
                                        onSignupFailed("Blank Response");
                                    }
                                }*/
                            } else {
